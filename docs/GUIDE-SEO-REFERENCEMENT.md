# Guide SEO & référencement — Smart Rénov (srs.cm)
...
- Le site doit être accessible en **HTTPS** sur : `https://srs.cm`
...
|----------|-------------|----------------|
| `NEXT_PUBLIC_SITE_URL` | **Oui** | `https://srs.cm` — URL canonique (sitemap, Open Graph, JSON-LD) |
...
```
NEXT_PUBLIC_SITE_URL=https://srs.cm
...
### Étape 2.4 — Lier le domaine `srs.cm`
...
- `https://srs.cm`
- `https://srs.cm/contact` (formulaire + CAPTCHA)
- `https://srs.cm/services`
...
1. Ouvrez : **https://srs.cm/sitemap.xml**
...
1. Ouvrez : **https://srs.cm/robots.txt**
2. Vérifiez la ligne : `Sitemap: https://srs.cm/sitemap.xml`
3. Vérifiez : `Disallow: /api/` et `Disallow: /videos/`

### Étape 3.3 — Manifest

1. Ouvrez : **https://srs.cm/manifest.webmanifest**
2. Le nom **Smart Rénov** doit apparaître.

### Étape 3.4 — Métadonnées d’une page

1. Sur `https://srs.cm/services/menuiserie-aluminium`, faites **clic droit → Afficher le code source**.
2. Cherchez :
...
2. Testez `https://srs.cm` — titre, description et image doivent s’afficher.
...
2. Connectez-vous avec un compte Google professionnel (ex. `info@srs.cm` si vous utilisez Google Workspace).

### Étape 4.2 — Ajouter la propriété

1. Cliquez **Ajouter une propriété**.
2. Choisissez **Préfixe d’URL** : `https://srs.cm`
...
|----------|-----|
| Haute | `https://srs.cm/` |
| Haute | `https://srs.cm/renovation-complete` |
| Haute | `https://srs.cm/contact` |
| Haute | `https://srs.cm/services` |
| Moyenne | `https://srs.cm/realisations` |
| Moyenne | `https://srs.cm/services/menuiserie-aluminium` |
| Moyenne | `https://srs.cm/about` |
...
1–2 fois par mois : photo de chantier + courte phrase + lien vers `https://srs.cm/realisations` ou `/contact`.
...
| Plateforme | Action |
|------------|--------|
| Facebook | Lien site = srs.cm, adresse, téléphone, description |
| LinkedIn | URL entreprise + site |
| TikTok / YouTube | Lien bio vers srs.cm |
...
2. Testez `https://srs.cm` en mode **Mobile** et **Bureau**.
...
### Technique
- [ ] Site en HTTPS sur `https://srs.cm`
- [ ] `NEXT_PUBLIC_SITE_URL=https://srs.cm` configuré
- [ ] Turnstile fonctionne sur `/contact`
- [ ] `https://srs.cm/sitemap.xml` accessible
- [ ] `https://srs.cm/robots.txt` accessible
...
- [ ] Google Business : photos + description + lien srs.cm
...
*Document créé pour Smart Rénov — srs.cm. Mettez à jour ce guide si l’URL, l’hébergeur ou les coordonnées changent.*
